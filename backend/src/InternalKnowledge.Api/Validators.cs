...
public sealed class KnowledgeEntryValidator:AbstractValidator<KnowledgeEntry>{public KnowledgeEntryValidator(){RuleFor(x=>x.Title).NotEmpty().MaximumLength(200);RuleFor(x=>x.Summary).NotEmpty().MaximumLength(2000);RuleFor(x=>x.OriginalInput).NotEmpty().MaximumLength(20000);RuleFor(x=>x.ConfidenceScore).InclusiveBetween(0,1);RuleForEach(x=>x.Tags).NotEmpty().MaximumLength(80);RuleForEach(x=>x.Technologies).NotEmpty().MaximumLength(80);RuleFor(x=>x.Tags).Must(x=>x.Count<=30);RuleFor(x=>x.Technologies).Must(x=>x.Count<=30);}}
public sealed class AskRequestValidator:AbstractValidator<AskRequest>{public AskRequestValidator(){RuleFor(x=>x.Question).NotEmpty().Length(3,4000);RuleFor(x=>x.History).Must(x=>x is null||x.Count<=10);RuleForEach(x=>x.History).ChildRules(turn=>{turn.RuleFor(x=>x.Role).Must(x=>x is "user" or "assistant");turn.RuleFor(x=>x.Content).NotEmpty().MaximumLength(4000);});}}
